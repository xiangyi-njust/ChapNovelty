...
class Dataset:
    ...
    def get_q_journal_list(self):
        items = []
        for year in tqdm(range(self.focal_year - 3, self.focal_year)):
            if self.client_name:
    ...
    def get_item_paper(self, ):
        ...
        for doc in tqdm(self.docs, desc="get_papers_item"):
            if self.list_ids:
                if doc[self.id_variable] in self.list_ids:
                    if self.sub_variable:
                        ...
                        self.papers_items.update({doc[self.id_variable]: doc_items})
                    else:
                        # self.papers_items.update({int(doc[self.id_variable]): doc[self.variable]})
                        self.papers_items.update({doc[self.id_variable]: doc[self.variable]})
            else:
                if self.sub_variable:
                    doc_items = list()
                    for item in doc[self.variable]:
    ...
    def sum_cooc_matrix(self, window=None):
        ...
        if window:
            ...
            try:
                ...
                files = [file for file in files if int(file.split("/")[-1].split(".")[0]) < self.focal_year]
            for file in tqdm(files, desc="Summing cooc"):
                if i == 0:
    ...
    def get_cooc(self):
        ...
        self.path_input = "../../Data/cooc_sec/{}/{}_{}/fast_uzzi_sec".format(self.variable, type1, type2)
        self.name2index = pickle.load(open(self.path_input + "/{}_name2index.p".format(self.focal_year), "rb"))
        ...
        if self.indicator == "foster":
            ...
            self.futur_adj = self.sum_cooc_matrix(
                window=range(self.focal_year + 1, self.focal_year + self.time_window_cooc + 1))

            print('Calculate difficulty matrix')
            self.difficulty_adj = self.sum_cooc_matrix(
                window=range(self.focal_year - self.time_window_cooc, self.focal_year))
        else:
...
class create_output(Dataset):
    ...
    def get_paper_score(self):
        ...
        elif not self.unique_pairwise and not self.keep_diag:
            combis = [(i, j) for i, j in combinations(self.current_items, 2) if i != j]
        elif self.unique_pairwise and not self.keep_diag:
        ...
        if self.indicator == 'wang':
            ...
            score = {'conventionality': np.nanmedian(self.scores_array),
                     'novelty': np.nanquantile(self.scores_array, 0.1)}
        ...
        if self.indicator == 'wang':
            self.comb_scores = pickle.load(
                open(
                    'Data/score/{}/{}/{}.p'.format(
                        self.indicator,
                        self.variable + '_' + str(self.time_window_cooc) + '_' + str(
                            self.n_reutilisation) + self.restricted,
                        self.focal_year),
                    "rb"))
        else:
            self.comb_scores = pickle.load(
                open(
                    '../../Data/score/fast_{}/{}/{}.p'.format(
                        self.indicator, self.variable, self.focal_year),
                    "rb"))

        list_of_insertion = []
        for idx in tqdm(list(self.papers_items), desc='start'):
            if self.indicator in ['uzzi', 'wang', 'lee', 'foster']:
                ...
                if len(self.papers_items[idx]) > 2:
                    ...
                    # Use novelty score of combination + Matrix of combi of papers
                    # to have novelty score of the paper with id_variable = idx
            ...
            if self.client_name:
                ...
                list_of_insertion.append(
                    {self.id_variable: idx, self.key: self.doc_infos, self.year_variable: self.focal_year})

        if self.client_name:
            self.collection_output.insert_many(list_of_insertion)
        else:
            list_of_insertion = json.dumps(list_of_insertion, indent=4)
            with open(self.path_output + "/{}.json".format(self.focal_year), 'w') as f:
                f.write(list_of_insertion)
    ...
    def update_paper_values(self, **kwargs):
        ...
        **kwargs : keyword arguments, optional
        ...
        if self.client_name:
            ...
            if self.indicator == "wang":
                self.collection_output_name = "output_" + self.indicator + "_" + self.variable + "_" + str(
                    self.time_window_cooc) + "_" + str(self.n_reutilisation) + self.restricted
            ...
            if self.indicator == "wang":
                self.path_output = "Result/{}/{}".format(self.indicator,
                                                         self.variable + "_" + str(self.time_window_cooc) + "_" + str(
                                                             self.n_reutilisation) + self.restricted)
            else:
                self.path_output = "Result/fast_{}_sec/{}/".format(self.indicator, self.variable)
